...
namespace zstdframe
{
    ...
    uint64_t GetTotalDecompressedSize(const uint8_t* src, size_t srcSize, std::string* error)
    {
        ...
        // Walk every frame, accumulating decompressed (content) sizes from the
        // frame headers. Skippable frames contribute nothing.
        while (offset < srcSize)
        {
            const size_t remaining = srcSize - offset;
            if (remaining < kStartingInputLength)
            ...
            if (zfh.frameType == FrameType::Zstd)
            {
                ...
                // Saturating add: a corrupt header set cannot wrap the budget to a small value.
                if (total > UINT64_MAX - zfh.frameContentSize)
                    return fail("total decompressed size overflows 64 bits");
